...
var url = "mongodb+srv://Examen:123@cluster0.lsscu.mongodb.net/POKEMONGAME?retryWrites=true&w=majority";
...
MongoClient.connect(url, function(err, db) {
  ...
  app.post("/add", async (req, res) => {
    ...
    dbo.collection("mazo").insertOne(pokecard);
    res.send("DONE");
  });
  ...
  app.get("/get/:pokename", (req, res) => {
    ...
    dbo.collection("mazo").find({name : name}).toArray(function(err, result){ res.send(result) });
  ...
  app.delete("/delete/:pokename", function (req, res) {
    let params = getParams(req);
    let name = params.pokename; 
    dbo.collection("mazo").remove({name : name})
    res.send("DELETED")
  });
  
  app.get("/get", function (req, res) {
    dbo.collection("mazo").find().toArray(function(err, result){ res.send(result) });
  });
  ...
  app.put("/put/:pokename", function (req, res) {
    ...
    dbo.collection("mazo").updateOne(myquery, newvalues, function(err, res) {
      if (err) throw err;
    });
    res.send("UPDATED");
  });
